...
std::vector<int64_t> factoriza(int64_t n, const criba& c) {      // suposición: n >= 2
   ...
   if (n <= c.tope) {
      ...
      for (int64_t i = 0; i < c.primos.size( ) && c.primos[i] <= raiz; ++i) {
      ...
      for (int64_t i = 2; ; i++) {
         ...
         if (checar != n) {
            std::vector<int64_t> factores1 = factoriza(checar, c);
            std::vector<int64_t> factores2 = factoriza(n / checar, c);
...
int main( ) {
   ...
   for (int i = 0; i < t; ++i) {
      int64_t actual;
      std::cin >> actual;
      auto factores = factoriza(actual, c);
