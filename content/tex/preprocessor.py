...
def processwithcomments(caption, instream, outstream, listingslang):
    knowncommands = ['Nombre','Autor', 'Fecha', 'Descripción', 'Fuente', 'Complejidad', 'Memoria', 'Licencia', 'Estado', 'Uso', 'Detalles']
    requiredcommands = ['Nombre','Autor', 'Descripción']
    includelist = []
    error = ""
    warning = ""
    ...
    for line in lines:
        if 'exclude-line' in line:
            continue
        include_line = 'include-line' in line
        if include_line:
            line = line.replace('// ', '', 1)
        had_comment = "///" in line
        ...
        if include is not None and not keep_include:
            if '"' not in include:
                continue
            include = include.strip('"').split('/')[-1]
            includelist.append(r"\nameref{code:%s}" % include)
            continue
        if 'main(' in line and not include_line:
            if nlines[-1] == '':
                del nlines[-1]
            break
        line = line.replace('   ', '\t')
    ...
    if error:
        print(r"\kactlerror{%s: %s}" % (caption, error), file=outstream)
    else:
        ...
        if commands.get("Memoria"):
            print(r"\defmemory{%s}" % ordoescape(commands["Memoria"]), file=outstream)
        if includelist:
            print(r"\defdependencies{%s}" % ", ".join(includelist), file=outstream)
        if nsource:
            print(r"\rightcaption{%s%d líneas}" % (hsh, len(nsource.split("\n"))), file=outstream)
        print(r"\begin{mycode}{%s}{%s}" % (escape(commands["Nombre"]), caption), file=outstream)
        print(r"\begin{minted}{%s}" % listingslang, file=outstream)
        print(nsource, file=outstream)
        print(r"\end{minted}", file=outstream)
        print(r"\end{mycode}", file=outstream)

def processraw(caption, instream, outstream, listingslang = 'text'):
    try:
        source = instream.read().strip()
        #addref(caption, outstream)
        print(r"\rightcaption{%d líneas}" % len(source.split("\n")), file=outstream)
        print(r"\begin{mycode}{%s}{%s}" % (pathescape(caption), caption), file=outstream)
        print(r"\begin{minted}{%s}" % listingslang, file=outstream)
...
def extract_name(file_path):
    name_pattern = re.compile(r"^\s*\*\s*Nombre:\s*(.+)\s*$", re.MULTILINE)
...
def print_external(outstream):
    # Get all subfolders and sort them by name
    sub_folders = sorted([f for f in os.scandir('codes') if f.is_dir()], key=lambda x: x.name)

    for sub_folder in sub_folders:
        print(r"\kactlcontentdir{%s}" % sub_folder.path, file=outstream)
        print(r"\chapter{%s}" % sub_folder.name, file=outstream)

        files = [f for f in os.scandir(sub_folder.path) if f.is_file() and extract_name(f.path)]
        files.sort(key=lambda f: extract_name(f.path))
        for file in files:
            print(r"\kactlimport{%s}" % file.name, file=outstream)

    print(r"\kactlcontentdir{content}", file=outstream)
...
def main():
    ...
    print_external_flag = False
    try:
        opts, args = getopt.getopt(sys.argv[1:], "ho:i:l:c:", ["help", "output=", "input=", "language=", "caption=", "print-header=", "print-external"])
        for option, value in opts:
            ...
            if option == "--print-external":
                print_external_flag = True
        if print_header_value is not None:
            print_header(print_header_value, outstream)
            return
        if print_external_flag:
            print_external(outstream)
            return
        # print(" * \x1b[1m{}\x1b[0m".format(caption))
        if language in ["cpp", "cc", "c", "h", "hpp"]:
            processwithcomments(caption, instream, outstream, 'cpp')
